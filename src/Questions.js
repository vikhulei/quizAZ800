const Questions = {
  ...
  Storage: [
    ...
    {q: 'Expplain File Sync Cloud tiering, Cloud backup and Disasdter recovery', a: 'Cloud tieering - only required files are stored on-prem, the rest are loaded as needed<br>Cliud backup - 200 snapshots, backup for up to 10 years<br>Recovery - install File Sync agent, register server, add to the group', tag: 0},
    {q: 'What are 5 steps to implement Azurre File Sync?', a: '1)Deploy the Storage Sync Service in Azure portal<br>Install File Sync agent on the on-prem server from MS Download Center<br>Register server with Syn Service<br>Create a sync group: 1 cloud to up to 50 on-prem; changes on-prem immed, in cloud - 24 hours<br>Add server endpoints', tag: 0},
